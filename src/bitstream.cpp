...
void BitWriter::write_bits(const string& bit_string) {
    for (char c : bit_string) write_bit(c == '1' ? 1 : 0);
}

string BitWriter::finish() {
    if (bit_count_ > 0) {
...
BitReader::BitReader(const string& data)
    : data_(data), byte_pos_(0), bit_pos_(7) {}

int BitReader::read_bit() {
    if (exhausted()) throw runtime_error("BitReader: read past end");
